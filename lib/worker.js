...
'use strict';
    ...
    var PREMIUM_RATE_LIMIT = 100000;
    var REGISTERED_RATE_LIMIT = 10000;
    var ANONYMOUS_RATE_LIMIT = 500;

    var currentRateLimit = ANONYMOUS_RATE_LIMIT;
...
Worker.prototype.runServer = function (config) {
    ...
    var getRateLimit = function(myWorker,req,callback)
    {
        if(req.headers.referer && req.headers.referer.split("/").length > 2 && req.headers.referer.split("/")[2])
        {
            myWorker.redisClient.sismember("registered",req.headers.referer.split("/")[2],function(err,obj){
                if(err)
                {
                    log("err sismembers " +  err);
                    callback(ANONYMOUS_RATE_LIMIT);
                }
                else
                {
                    log("found " + req.headers.referer.split("/")[2] + " obj: " + obj);
                    if(obj==1)
                    {
                        callback(REGISTERED_RATE_LIMIT);
                        log("registered request")
                    }
                    else
                    {
                        callback(ANONYMOUS_RATE_LIMIT);
    ...
    var httpRequestHandler = function (req, res) {
        ...
        var findAndValidateVersion = function(url)
        {
            var tokens = req.url.split("/");
            if(tokens.length < 2)
                return {result: false,version:null};
            else 
                return {result: validateVersion(tokens[1]),version: tokens[1]};
        }
        ...
        var proxyRequest = function () {
            ...
            log("req.headers.referer: " + req.headers.referer + "req.headers.origin: " + req.headers.origin + " req.headers.host: " + req.headers.host);
            
            var currentRequest=this;
            ratelimit.add(ip);
            getRateLimit(this,req,function(currentRateLimit){
                ratelimit.count(ip, 60, 
                    function (err, count) {
                       
                        if (count > currentRateLimit)
                        {
                            var body = 'you have reached the rate limit for your account of ' +  currentRateLimit;
                            res.writeHead(200,{
                        ...
                        {
                           log("in last 60 seconds, " + ip + " has made " + count + " requests" );
                           var version = findAndValidateVersion(req.url);
                           log("API version: " + JSON.stringify(version) + " host: " + (version.result?req.headers.host+":"+version.version:req.headers.host));

                            currentRequest.cache.getBackendFromHostHeader((version.result?req.headers.host+":"+version.version:req.headers.host), function (err, code, backend) {
                            ...
                            if(version.result){
                                var idx = req.url.indexOf(version.version);
                                log("idx: " + idx);
                                var newUrl = req.url.slice(idx+8,req.url.length);
                                log("Original req.path: " + req.path + " req.url: " + req.url + " newUrl: " + newUrl);
                                req.path = req.url = newUrl;
                                log("Modified req.path: " + req.path + " req.url: " + req.url);
                            }
